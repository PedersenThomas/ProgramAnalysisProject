...
public class Util {
    ...
    static {
        ...
        all.add(Signs.negative); all.add(Signs.zero); all.add(Signs.positive);
        ALL = new PowerSetOfSigns(all);

        PowerSetOfSigns[][] additionTable = {
                {NEGATIVE_ONLY, NEGATIVE_ONLY, ALL},
                {NEGATIVE_ONLY, ZERO_ONLY, POSITIVE_ONLY},
                {ALL, POSITIVE_ONLY, POSITIVE_ONLY}
        };
        ADDITION_TABLE = additionTable;

        PowerSetOfSigns[][] subtractionTable = {
                {ALL, NEGATIVE_ONLY, NEGATIVE_ONLY},
                {POSITIVE_ONLY, ZERO_ONLY, NEGATIVE_ONLY},
                {POSITIVE_ONLY, POSITIVE_ONLY, ALL}
        };
        SUBTRACTION_TABLE = subtractionTable;

        PowerSetOfSigns[][] multiplicationTable = {
                {POSITIVE_ONLY, ZERO_ONLY, NEGATIVE_ONLY},
                {ZERO_ONLY, ZERO_ONLY, ZERO_ONLY},
                {NEGATIVE_ONLY, ZERO_ONLY, POSITIVE_ONLY}
        };
        MULTIPLICATION_TABLE = multiplicationTable;

        PowerSetOfSigns[][] divisionTable = {
                {POSITIVE_ONLY, EMPTY_SET, NEGATIVE_ONLY},
                {ZERO_ONLY, EMPTY_SET, ZERO_ONLY},
	...
	public static PowerSetOfSigns evalDSArithmeticExpression(
        ...
        if (expression instanceof Identifier) {
            ...
            PowerSetOfSigns signsOfIndex =
                    evalDSArithmeticExpression(index, signState);
            if (signsOfIndex.equals(EMPTY_SET)
                    || signsOfIndex.equals(NEGATIVE_ONLY)) {
            ...
            PowerSetOfSigns leftResult =
                    evalDSArithmeticExpression(arithOp.getLeft(), signState);
            PowerSetOfSigns rightResult =
                    evalDSArithmeticExpression(arithOp.getRight(), signState);
            ...
            PowerSetOfSigns rightResult =
                    evalDSArithmeticExpression(right, signState);
    ...
    private static PowerSetOfSigns combine(
            ArithmeticOperator operator, PowerSetOfSigns left, PowerSetOfSigns right) {
        ...
        for (Signs leftSign : left.getSigns()) {
            for (Signs rightSign : right.getSigns()) {
                switch (operator) {
                    case Plus:
                        result.addAll(ADDITION_TABLE
                                [toIndex(leftSign)][toIndex(rightSign)]
                                .getSigns());
                        break;
                    case Minus:
                        result.addAll(SUBTRACTION_TABLE
                                [toIndex(leftSign)][toIndex(rightSign)]
                                .getSigns());
                        break;
                    case Multiply:
                        result.addAll(MULTIPLICATION_TABLE
                                [toIndex(leftSign)][toIndex(rightSign)]
                                .getSigns());
                        break;
                    case Divide:
                        result.addAll(DIVISION_TABLE
                                [toIndex(leftSign)][toIndex(rightSign)]
                                .getSigns());
                        break;
                    default:
